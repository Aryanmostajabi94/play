...
// Screen C4 — Request Sent.
// Shown at /booking/[id] when booking.status === "pending" (request-type
// bookings, including large-group overrides, await venue confirmation).
export default function BookingRequestSent({ booking }: { booking: BookingWithVenue }) {
  const deadline = booking.confirmation_deadline
    ? new Date(booking.confirmation_deadline)
    : null;
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 15, marginBottom: 6 }}>{booking.venue.name}</div>
      <div style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 18 }}>
        The venue will confirm your request
        {deadline
          ? ` by ${deadline.toLocaleString(undefined, {
              weekday: "short",
              hour: "numeric",
              minute: "2-digit",
            })}.`

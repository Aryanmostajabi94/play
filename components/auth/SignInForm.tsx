...
//
// All three OAuth buttons are wired end-to-end on the code side; each only
// works once the matching provider is switched on in the Supabase
// dashboard (Authentication > Providers) with that provider's own OAuth
// credentials — see app/actions/auth.ts for the same caveat, structurally
// identical to the Stripe-test-keys situation on D1. None of the three are
// configured yet, so right now all three bounce back here with an error
// banner instead of actually signing in.
export default function SignInForm({ providerError }: { providerError?: string }) {
  ...
  return (
    <div>
      ...
      {/* OAuth path is the fastest way in for most people. Siblings of
          the email/password form below, not nested inside it — a nested
          <form> tag gets silently dropped by the HTML parser, so the
          button would otherwise attach to (and try to submit) the
          email/password form instead of its own action. */}
